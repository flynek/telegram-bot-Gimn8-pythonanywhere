...
back_counter = 0
kb_dict = [kb_start, kb_parents, kb_course]
message_dict = [dict_messages['st_m'], dict_messages['pr_m'], dict_messages['cr_m']]


@dp.callback_query_handler(text_startswith=['st_', 'pr_', 'cr_', 'back'])
async def start_func(callback: types.CallbackQuery):
    global previous_kb, previous_message, back_counter
    image_sign = InputMediaPhoto(open('photos\\sign.jpg', 'rb'))
    if callback.data == 'back':
        back_counter += 1
        if back_counter >= 2:
            index = kb_dict.index(previous_kb)
            previous_kb = kb_dict[index-1]
            previous_message = message_dict[index-1]
        ...
        if callback.data[:3] == 'st_':
            ...
            if callback.data == 'st_holidays':
                await edit_message_photo(call=callback, photo=image_sign, caption=dict_messages.get('hol_m'),
                                         kb=kb_back)
                await callback.answer()
                ...
                await edit_message_photo(call=callback, photo=image_sign, caption=dict_messages.get('pr_m'),
                                         kb=kb_parents)
                await callback.answer()
            ...
            if callback.data == 'pr_class_1_doc':
                ...
                await edit_message_photo(call=callback, photo=image_sign, caption=dict_messages.get('cr_m'),
                                         kb=kb_course)
                await callback.answer()
            ...
            if callback.data == 'cr_register_course':
                ...
                await edit_message_photo(call=callback, photo=image_sign, caption=dict_messages.get('o_cr_m'),
                                         kb=kb_back)
                await callback.answer()
            elif callback.data == 'cr_pay_course':
                await edit_message_photo(call=callback, photo=image_sign, caption=dict_messages.get('p_cr_m'),
                                         kb=kb_back)
                await callback.answer()
